...
def find_entries(db_session: Session, user_id: int) -> Sequence[Entry]:
    """Find all (non-pinned) entries"""
    return db_session.query(Entry).filter_by(
        user_id=user_id, pinned=False)


def find_link(db_session: Session, user_id: int) -> Sequence[Link]:
    return db_session.query(Link).filter_by(user_id=user_id)


def find_pinned_entry(db_session: Session, user_id: int) -> Entry:
    assert isinstance(user_id, int)
    return db_session.query(Entry).filter_by(
        user_id=user_id, pinned=True).one()


def find_user(db_session: Session, user_id: int) -> User:
    return db_session.query(User).filter_by(id=user_id).one()


def verify_pinned_entry(db_session: Session, pinned_entry: Entry, old_password: str) -> None:
...
def reencrypt_entry(db_session: Session, entry: Entry, old_password: str, new_password: str) -> bytes:
...
def reencrypt_link(db_session: Session, link: Link, old_password: str, new_password: str) -> bytes:
...
def reencrypt_entries(db_session: Session, user_id: int, old_password: str, new_password: str):
    """
    ...
    for entry in find_entries(db_session, user_id):
        entry_key = reencrypt_entry(db_session, entry, old_password, new_password)
...
def reencrypt_links(db_session: Session, user_id: int, old_password: str, new_password: str):
    # guaranteed to exist here
    enc_keys_db = db_session.query(EncryptionKeys).filter_by(user_id=user_id).one()
    keys_db = enc_keys_db.decrypt(old_password)
    for link in find_link(db_session, user_id):
        link_key = reencrypt_link(db_session, link, old_password, new_password)
...
def reencrypt_keys_db(db_session: Session, user_id: int, old_password: str, new_password: str):
...
def change_password_in_user_table(db_session: Session, user_id: int, new_password: str) -> None:
...
def change_password(db_session: Session, user_id: int, old_password: str, new_password: str) -> bool:
    ...
    try:
        pinned_entry = find_pinned_entry(db_session, user_id)
        verify_pinned_entry(db_session, pinned_entry, old_password)
    ...
    reencrypt_entries(db_session, user_id, old_password, new_password)
    reencrypt_links(db_session, user_id, old_password, new_password)
    reencrypt_keys_db(db_session, user_id, old_password, new_password)
    change_password_in_user_table(db_session, user_id, new_password)
    create_pinned_entry(db_session, user_id, new_password)
    db_session.commit()
    return True
